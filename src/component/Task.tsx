  ...
  const Task = ({id, name, deleteFn,doneFn} : TaskProps) => {
    ...
    >
      ...
      <div className="flex space-x-1 items-center opacity-0 group-hover:opacity-100 ">
        <button className="bg-green-400 font-serif w-24 text-2xl" onClick={ () => doneFn(id) } >Done</button>
        <button className="bg-red-400 font-serif w-24 text-2xl" onClick={ () =>  deleteFn(id)} >Delete</button>
      </div>
    </div>
    )
  }
